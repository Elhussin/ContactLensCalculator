...
class EyeContactLens(QWidget):
    ...
    def init_ui(self):
        ...
        self.result_header= ["R/L", "Exact SPH", "Exact CY", "SPH", "CY", "AX", "ADD", "BV"]
        self.table = self.create_table(2, 8, self.header)  # Create the input data table
        self.result_table = self.create_table(0, 8, self.result_header)  # Create the result table
        ...
        footer = self.create_footer() #Creat  Footer to LAyout 
        layout.addWidget(footer)  #Add Footer to the layout
        ...
        self.add_labels_to_first_column()  # Add labels for left and right eyes in the first column


    def create_table(self, rows, cols, headers):
        """
        ...
        table.setMaximumHeight(150 if rows == 2 else 300)  # Set the maximum height depending on the number of rows
        return table
    ...
    def add_labels_to_first_column(self):
        """
        Adds labels to the first column of the table to represent the left (L) and right (R) eyes.
        """
        self.table.setCellWidget(0, 0, QLabel("R"))  # Label for Right eye in the first row
        self.table.setCellWidget(1, 0, QLabel("L"))  # Label for Left eye in the second row
    ...
    def update_result_table(self, lens_data):
        ...
        This function populates the result table with lens data (Spheric and Toric), 
        adding rows and applying specific styling to certain cells. The table is first cleared 
        before adding new data, and custom styles are applied based on the header and values.

        Args:
            lens_data (list): A list of dictionaries containing the lens data (Spheric and Toric).
        """
        ...
        for row_data in lens_data:
            ...
            for col_idx, header in enumerate(self.result_header):
                ...
                if header == "Exact SPH" or header == "Exact CY":
                    # Apply a specific background color for "Exact SPH" and "Exact CY"
                    item.setBackground(QColor(255, 255, 100))  # Yellow background for Exact SPH and Exact CY
                elif header == "R/L":
                    item.setBackground(QColor(255, 255, 255))  # White background for R/L
                elif cell_value:
                    item.setBackground(QColor(200, 255, 200))  # Green background for valid values
                else:
